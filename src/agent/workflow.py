...
def build_graph():  #-> CompiledStateGraph:
    ...
    Graph flow:
    - START -> router
    - router -> researcher or reasoner (decided by router)
    - researcher -> reasoner
    - reasoner -> generator
    ...
    Returns:
        CompiledStateGraph: A compiled state graph that can be executed to process queries
    ...
    builder.add_node("reasoner", node.reasoner_node)
    builder.add_node("researcher", node.researcher_node)
    builder.add_node("generator", node.generator_node)
    builder.add_node("verifier", node.verifier_node)
